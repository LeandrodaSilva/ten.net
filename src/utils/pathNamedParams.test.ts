...
Deno.test("pathNamedParams - should extract multiple named parameters", () => {
  const result = pathNamedParams('/users/123/posts/456', '/users/[id]/posts/[postId]');
  assertEquals(result, { id: '123', postId: '456' });
});

Deno.test("pathNamedParams - should handle mixed static and dynamic segments", () => {
  const result = pathNamedParams('/api/v1/users/789/profile', '/api/v1/users/[userId]/profile');
...
Deno.test("pathNamedParams - should handle complex nested parameters", () => {
  const result = pathNamedParams('/org/abc/repos/xyz/issues/999', '/org/[orgId]/repos/[repoId]/issues/[issueId]');
